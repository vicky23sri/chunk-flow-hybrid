...
import { Database, CheckCircle2, Play, RefreshCw, AlertCircle, Unplug } from 'lucide-react';
...
export default function WorkflowHeader({
  ...
  const hasPgNode = nodes.some((n) => n.subtype === 'postgres');
  const hasS3Node = nodes.some((n) => n.subtype === 's3');
  const hasConnection = connections && connections.length > 0;
  const unconfiguredPg = nodes.filter((n) => n.subtype === 'postgres' && !n.isValid);
  const unconfiguredS3 = nodes.filter((n) => n.subtype === 's3' && !n.isValid);

  const missingRequirements = [];
  if (nodes.length === 0) {
    missingRequirements.push('Place Database Source & Amazon S3 Vault nodes on canvas');
  } else {
    if (!hasPgNode) missingRequirements.push('Add a Database Source node to canvas');
    if (!hasS3Node) missingRequirements.push('Add an Amazon S3 Vault node to canvas');
    if (!hasConnection) missingRequirements.push('Connect Blue dot on Source to Green dot on S3 with a wire');
    if (unconfiguredPg.length > 0) missingRequirements.push('Click "Test Connection" & "Save Config" on Database Source');
    if (unconfiguredS3.length > 0) missingRequirements.push('Click "Test S3 Configuration" & "Save Config" on S3 Vault');
  }
